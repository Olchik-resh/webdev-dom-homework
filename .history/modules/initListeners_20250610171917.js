...
Ваш код уже хорошо структурирован и обрабатывает добавление комментария. Вот улучшенная версия с добавлением обработки ошибок и некоторыми комментариями:
  ...
  addButton.addEventListener('click', () => {
    ...
    postComment(sanitizeHtml(name.value), sanitizeHtml(text.value))
      .then((data) => {
        ...
        // После добавления комментария, получаем обновлённый список комментариев
        return fetchComments();
      })
      .then((comments) => {
        // Здесь вы можете обработать полученные комментарии
