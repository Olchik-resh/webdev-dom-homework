...
const handleLikeClick = (renderComments) => {
  const likeButtons = document.querySelectorAll('.like-button');
  console.log('Количество кнопок "лайк":', likeButtons.length);
  ...
  for (const likeButton of likeButtons) {
    ...
    likeButton.addEventListener('click', (event) => {
      ...
      console.log('Index:', index); // Проверка индекса
          if (!comments[index]) {
          console.error(`Комментарий с индексом ${index} не найден`);
          return;
        }
...
renderComments();
handleLikeClick(renderComments);
